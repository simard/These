			...
			\begin{mysection}[sec-exp1-Resultats]{Résultats}
				...
				\begin{myfigure}
					...
					\begin{myps}(-1.25,-150)(10,475)
						\myaxes(0,10){résidus}(0,400)[100]{time~(s)}
						\myboxplot{exp1-timeaudio-residue-searchselection.csv}
						\mylegend{\myleg{Recherche}{mylightestblue}\myand\myleg{Sélection}{mylightblue}}
					\end{myps}
					\mycaption[fig-exp1-TempsDeRechercheEtDeSelectionComparesParResidu]{Temps de recherche et de sélection comparés par résidu}
				\end{myfigure}
				\begin{myfigure}
					\psset{xunit=0.075\textwidth,yunit=0.2cm}
					\begin{myps}(-1.25,-5)(12,21)
						\myaxes(0,12){résidus}(0,20)[4]{distance~(mm)}
						\mybarplot[barstyle=fourth-barstyle]{exp1-diff-groups1.csv}
						\mybarplot[header=false,barstyle=third-barstyle]{exp1-diff-groups2.csv}
						\mybarplot[header=false,barstyle=second-barstyle]{exp1-diff-groups3.csv}
					...
					\end{myps}
					\mycaption[fig-exp1-DistanceMoyenneEntreLesSujetsPourChaqueBinome]{Distance moyenne entre les sujets pour chaque binôme}
				\end{myfigure}
			\end{mysection}
		\end{mychapter}
