			...
			\begin{mysubsection}[sse-sota-CommunicationEnEnvironnementVirtuel]{Communication en environnement virtuel}
				...
				\begin{description}
					...
					\item[\myemph{Feedthrough}] C'est un canal de communication indirect où les participants communiquent entre eux par l'intermédiaire des artefacts \mycite{Dix-2004}.
						...
						\mycite[author]{Junuzovic-2009} utilise même le terme \myemph{feedthrough} pour une communication indirecte avec un participant en intermédiaire.
				...
				Le premier scénario est la collaboration à plusieurs utilisateurs \myref*{fig-sota-CollaborationAPlusieursUtilisateurs}.
				C'est l'aspect le plus intuitif de la distribution cognitive des charges \myref*{sse-sota-LaDistributionCognitiveDesChargesDeTravail}.
				Dans ce scénario, chaque participant possède un accès libre aux artefacts et est capable d'analyser l'environnement par l'observation ou par les retours sensoriels des artefacts sur les participants \mycite{Dewan-1998,Grasset-2002}.
				On notera que tous les participants accèdent aux mêmes artefacts ce qui peut poser une concurrence à l'accès des ressources comme signalé par \mycite[author]{Buisine-2011}.
				...
				On peut également identifier les scénarios multi-experts \myref*{fig-sota-CollaborationAPlusieursExperts}.
				Certaines tâches complexes (dont le \myglos{glo-DockingMoleculaire} peut nécessiter la présence d'experts venant de différents domaines.
				Dans ce scénario, les experts ont des connaissances différentes, potentiellement complémentaires \mycite{Althoff-2007a,Althoff-2007b}.
				De ce fait, leur spécialité nécessite un artefact adapté et chaque expert possède un accès à son artefact.
				Les artefacts se synchronisent entre eux afin de conserver un environnement virtuel cohérent pour tous les experts.
				Dans ce genre de scénarios, la compréhension partagée est difficile car les bases de connaissances des experts peuvent être très différentes \mycite{Bach-2010}.
				...
				Pour finir, on identifie le scénario à un manipulateur et un observateur \myref*{fig-sota-CollaborationParManipulateurEtObservateur}.
				L'idée est d'éviter les accès concurrents aux artefacts en ne fournissant l'accès qu'à un seul des utilisateurs.
				La difficulté de cette configuration vient de la nécessiter pour l'observateur de pouvoir agir sur l'artefact par l'intermédiaire du manipulateur.
				...
				\end{myfigure}
			\end{mysubsection}
			\begin{mysubsection}[sse-sota-PresenceAuSeinDuGroupe]{Présence au sein du groupe}
			\end{mysubsection}
