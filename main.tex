		...
		\begin{mychapter}[cha-DeformationCollaborativeDeMolecule]{Déformation collaborative de molécule}
			...
			\begin{mysection}[sec-exp2-Resultats]{Résultats}
				...
				\begin{mysubsection}[sse-exp2-GainDePerformanceSurLesTachesComplexes]{Gain de performance sur les tâches complexes}
					...
					\begin{myfigure}
						\psset{xunit=0.222222222\textwidth,yunit=1.5cm}
						\begin{mysubfigure}[\textwidth]
							\begin{myps}(-0.5,-0.75)(4,3)
								\myaxes(0,4){scénario}(0,2.5)[0.5]{temps~(s)}
					...
					La \myref{fig-exp2-DistancePassiveEtActiveEntreLesEffecteursTerminauxSurChaqueScenarioEnFonctionDuNombreDeSujets} présente les distances passives et actives \myvard{1} des différents scénarios \myvari{2} en fonction du nombre de sujets \myvari{1} ayant réalisé l'expérience.
					En regroupant les scénarios par classe de complexité, l'analyse montre qu'il n'y a pas d'effet significatif du nombre de sujets \myvari{1} sur la distance passive \myvard{1} pour les scénarios \myscenario{1a} et \myscenario{2a} (\myanova{exp2-passive-task-group-anova-trpzipper.tex}) ainsi que sur les scénarios \myscenario{1b} et \myscenario{2b} (\myanova{exp2-passive-task-group-anova-trpcage.tex}).
					Cependant, on constate un effet significatif du nombre de sujets \myvari{1} sur la distance active \myvard{1} pour les scénarios \myscenario{1a} et \myscenario{2a} (\myanova{exp2-active-task-group-anova-trpzipper.tex}) ainsi que sur les scénarios \myscenario{1b} et \myscenario{2b} (\myanova{exp2-active-task-group-anova-trpcage.tex}).
				\end{mysubsection}
