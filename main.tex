...
\begin{document}
	...
	\begin{mypart}[prt-Introduction]{Introduction}
		\begin{mychapter}[cha-LeSujet]{Le sujet}
			...
			\end{mysection}
		\end{mychapter}
		\begin{mychapter}[cha-ShaddockPlateFormeCollaborativeDeManipulationMoleculaire]{\myShaddock\ -- Plate-forme collaborative de manipulation moléculaire}
			\begin{mysection}[sec-Shaddock-Introduction]{Introduction}
				Le \myref{cha-LeSujet} nous a permis d'identifier les problématiques de recherche.
				C'est autour de ces problématiques que la plate-forme \myShaddock a été élaborée.

				Nous commencerons par présenter les choix de matériel et d'architecture logicielle \myref*{sec-Shaddock-ArchitectureMaterielleEtLogicielle}.
				...
				L'ensemble des éléments de la plate-forme sont organisés selon un architecture client/serveur; les raisons de ce choix sont expliqués dans la \myref{sse-Shaddock-UneArchitectureClientServeur}.

				Ensuite, la plate-forme de simulation moléculaire en temps-réel sera présentée \myref*{sec-Shaddock-PlateFormeDeSimulationEtDeVisualisation}.
				Tout d'abord, un logiciel de visualisation complet est nécessaire pour obtenir des affichages détaillés et complexes de molécules; le logiciel sera présenté dans la \myref{sse-Shaddock-LogicielDeVisualisationMoleculaire}.
				...
				Enfin, un module spécifique permet d'obtenir des simulations moléculaires en temps-réel et est présenté dans la \myref{sse-Shaddock-PlateFormeDeSimulationMoleculaireEnTempsReel}.
				...
				Ce plus, notre étude du travail collaboratif a mené à la proposition de nouveaux outils haptiques présentés dans la \myref{sse-Shaddock-LesNouveauxOutilsDInteraction}.
			...
			\begin{mysection}[sec-Shaddock-ArchitectureMaterielleEtLogicielle]{Architecture matérielle et logicielle}
				\begin{mysubsection}[sse-Shaddock-LInterfaceHaptique]{L'interface haptique}
					...
					De plus, le but final de cette thèse est d'apporter des outils d'assistance haptique pour le travail collaboratif.
					Les types d'interfaces haptiques disponibles sur le marché sont relativement nombreuses et variées.
					Cependant, certaines contraintes nous ont permis de choisir le \myOmni.
					...
					Cependant, il est également nécessaire que le périphérique possède trois \myacro*{acr-DDL} en retour haptique au minimum afin de pouvoir générer des outils d'assistance haptique.
					...
					Cependant, nous verrons dans la \myref{sse-Shaddock-UneArchitectureClientServeur} que c'est un serveur spécifique qui s'occupera de cette communication avec l'interface.
				\end{mysubsection}
				\begin{mysubsection}[sse-Shaddock-ConfigurationDeTravailCollaboratif]{Configuration de travail collaboratif}
					Nous avons vu dans le \myref{cha-LeSujet} que les conditions de travail collaboratif peuvent être de plusieurs types.
					L'\myacro{acr-EVC} proposé doit permettre de répondre à chacune de nos attentes et deux critères sont primordiaux.
					...
					\mycite[author]{Casera-2006} privilégie une vue publique à tous les utilisateurs et améliore ainsi la conscience périphérique dans une tâche collaborative.
				...
				\begin{mysubsection}[sse-Shaddock-UneArchitectureClientServeur]{Une architecture client/serveur}
					...
					D'ailleurs, \mycite[author]{Kim-2004} qui étudie le déplacement collaboratif d'une boîte assisté par l'haptique signalent qu'ils ont dû ajouter une viscosité importante dans le retour haptique pour éviter les instabilités.
					...
					\begin{mysubsubsection}[sss-interaction-VRPN]{\myacronl+{acr-VRPN}}
						Afin de gérer ces connexions client/serveur, nous utilisons le logiciel \myacro{acr-VRPN} développé par \mycite[author]{Taylor-II-2001}.

						\myacro{acr-VRPN} offre un moyen simple et relativement universel de connecter un périphérique.
						...
						Dans notre cas, l'interface haptique est connectée physiquement à un ordinateur et un serveur \myacro{acr-VRPN} régit cette interface.
						C'est seulement par l'intermédiaire de ce serveur \myacro{acr-VRPN} et à travers le réseau que le client (\myacro{acr-VMD} dans notre cas) va envoyer et recevoir les informations à l'interface haptique.

						La compilation de \myacro{acr-VRPN} en tant que serveur de \myOmni sous le système d'exploitation \myLinux (\myUbuntu $v10.04$) a nécessité quelques modifications dans le code.
						...
						C'est pourquoi aucune des expérimentations proposées ne dure plus de \mynum[mn]{30} ou, le cas échéant, une pause est effectuée au bout de \mynum[mn]{30} afin d'aérer la salle d'expérimentation.
					\end{mysubsubsection}
				\end{mysubsection}
			\end{mysection}
			\begin{mysection}[sec-Shaddock-PlateFormeDeSimulationEtDeVisualisation]{Plate-forme de simulation et de visualisation}
				...
				Le logiciel de simulation est présenté dans la \myref{sse-Shaddock-LogicielDeSimulationMoleculaire}.
				...
				Un module présenté dans la \myref{sse-Shaddock-PlateFormeDeSimulationMoleculaireEnTempsReel} permet de connecter le logiciel de visualisation avec le logiciel de simulation afin d'obtenir une simulation en temps-réel.
				...
				\end{mysubsection}
				\begin{mysubsection}[sse-Shaddock-PlateFormeDeSimulationMoleculaireEnTempsReel]{Plate-forme de simulation moléculaire en temps-réel}
			...
			\begin{mysection}[sec-Shaddock-LesOutilsDInteraction]{Les outils d'interaction}
				...
				\begin{mysubsection}[sse-Shaddock-LesNouveauxOutilsDInteraction]{Les nouveaux outils d'interaction}
					...
					\end{mysubsubsection}
					\begin{mysubsubsection}[sss-Shaddock-OutilDeDesignationEtAttraction]{Outil de désignation et attraction}
	...
	\begin{mypart}[prt-EtudeDuTravailCollaboratif]{Étude du travail collaboratif}
		\begin{mychapter}[cha-RechercheCollaborativeDeResidusEnSimulationMoleculaire]{Recherche collaborative de résidus en simulation moléculaire}
			...
			\begin{mysection}[sec-exp1-DispositifExperimentalEtMateriel]{Dispositif expérimental et matériel}
				...
				L'expérience est basée sur l'\myacro{acr-EVC} présenté dans le \myref{cha-ShaddockPlateFormeCollaborativeDeManipulationMoleculaire}.
		...
		\begin{mychapter}[cha-LesDynamiquesDeGroupe]{Les dynamiques de groupe}
			...
			\begin{mysection}[sec-exp3-DispositifExperimentalEtMateriel]{Dispositif expérimental et matériel}
				...
				\begin{mysubsection}[sse-exp3-DispositifTechnique]{Dispositif technique}
					...
					Quatre interfaces haptiques se trouvent faces aux sujets : ce sont quatre outils de déformation \mytool{tug} \myref*{par-simulation-LesOutilsDeManipulation}.
	...
	\begin{mypart}[prt-PropositionsPourLeTravailCollaboratif]{Propositions pour le travail collaboratif}
		\begin{mychapter}[cha-TravailCollaboratifAssisteParHaptique]{Travail collaboratif assisté par haptique}
			...
			\begin{mysection}[sec-exp4-DispositifExperimentalEtMateriel]{Dispositif expérimental et matériel}
				...
				Pour les détails techniques concernant la plate-forme et les outils de manipulation et de déformation, se reporter au \myref{cha-ShaddockPlateFormeCollaborativeDeManipulationMoleculaire}.
						...
						\begin{myparagraph}[par-exp4-EvaluationQualitativeDuSysteme]{\myvard{4} Évaluation qualitative du système}
							...
							\begin{myTodo}{Le questionnaire}{Écrire le questionnaire soumis au sujets}
								...
								\begin{enumerate}
									...
									\item Pensez-vous avoir une position de \myglos{glo-Meneur} dans la configuration collaborative ?
