...
\begin{document}
	...
	\begin{mychapter}[cha-pro-DispositifExperimental]{Dispositif expérimental}
		...
		\begin{mysection}[sec-pro-OutilsDeManipulation]{Outils de manipulation}
			...
			Ce sera par l'intermédiaire d'un outil \mytool{grab} dans la première expérimentation \myref*{sse-pro-exp1-Materiel}, par une souris~\myThreeD dans la seconde \myref*{sse-pro-exp2-Materiel} puis par une simple souris \myUSB dans la dernière expérimentation \myref*{sse-pro-exp4-Materiel}.
		\end{mysection}
		\begin{mysection}[sec-pro-exp1-SpecificitesDeLaPremiereExperimentation]{Spécificités de la première expérimentation}
			\begin{mysubsection}[sse-pro-exp1-Materiel]{Matériel}
				...
				Cependant, un outil d'orientation de la molécule est mis à disposition pour des raisons détaillés dans la \myref{sse-pro-exp1-OutilsDeManipulation}.
				...
				La \myref{fig-pro-exp1-SchemaDuDispositifExperimental} est un schéma récapitulatif de la disposition des tous les éléments dans la salle d'expérimentation.
				La \myref{fig-pro-exp1-PhotographieDuDispositifExperimental} est une photographie de la salle d'expérimentation.

				\begin{myfigure}
					\myimage{exp1-schema}
					\mycaption[fig-pro-exp1-SchemaDuDispositifExperimental]{Schéma du dispositif expérimental}
				\end{myfigure}
				\begin{myfigure}
					\myimage{exp1-photo}
					\mycaption[fig-pro-exp1-PhotographieDuDispositifExperimental]{Photographie du dispositif expérimental}
				\end{myfigure}
			\end{mysubsection}
			\begin{mysubsection}[sse-pro-exp1-Visualisation]{Visualisation}
				...
				\end{myfigure}
			\end{mysubsection}
			\begin{mysubsection}[sse-pro-exp1-OutilsDeManipulation]{Outils de manipulation}
				Durant une tâche de recherche, la déformation seule sur une molécule de taille importante va limiter les possibilités de recherche des sujets.
				En effet, tout \myglos{glo-Residu} qui se trouverait derrière la molécule ne pourrait être trouvé que lorsque toute la partie sur le devant aura été déformée et écartée.
				C'est pourquoi, nous mettons un outil d'orientation de la molécule à disposition des sujets.

				Cet outil, concrétisé par une interface haptique associé à l'outil \mytool{grab}, permet de sélectionner la molécule puis de déplacer et de l'orienter.
			...
			\end{mysubsection}
		\end{mysection}
		\begin{mysection}[sec-pro-exp2-SpecificitesDeLaPremiereExperimentation]{Spécificités de la seconde expérimentation}
			\begin{mysubsection}[sse-pro-exp2-Materiel]{Matériel}
				...
				Les \myref{fig-pro-exp2-SchemaDuDispositifExperimental} et \myref{fig-pro-exp2-PhotographieDuDispositifExperimental} illustrent par un schéma et une photographie le dispositif expérimental.

				\begin{myfigure}
					\myimage{exp2-schema}
					\mycaption[fig-pro-exp2-SchemaDuDispositifExperimental]{Schéma du dispositif expérimental}
				\end{myfigure}
				\begin{myfigure}
					\myimage{exp2-photo}
					\mycaption[fig-pro-exp2-PhotographieDuDispositifExperimental]{Photographie du dispositif expérimental}
				\end{myfigure}
			\end{mysubsection}
			\begin{mysubsection}[sse-pro-exp2-Visualisation]{Visualisation}
				...
				Des images représentants les différents scénarios dans leur état initial sont présentées.
				...
				\end{myfigure}
			\end{mysubsection}
			\begin{mysubsection}[sse-pro-exp2-OutilsDeManipulation]{Outils de manipulation}
				...
				Grâce au choix du matériel capable de différencier aisément les opérations de translation et les opérations de rotation, nous avons choisi de ne conserver que les \myacro*{acr-DDL} en rotation.
				En effet, la molécule n'a pas vraiment besoin d'être déplacer à l'écran et c'est surtout la possibilité d'orienter la molécule qui est nécessaire aux sujets.
				Ceci permet également d'éviter des molécules qui sortent de l'écran à cause d'une mauvaise manipulation.
				De plus, cela décharge d'une charge cognitive inutile.
				...
				Le \myglos{glo-Residu} correspondant sur la molécule stable est également mis en surbrillance comme expliqué sur la \myref{fig-pro-exp2-IllustrationDesRendusPourLAffichageDeLaMolecule}.
				Le \myglos{glo-Residu} de la molécule stable est représenté par un rendu \myCPK avec des atomes grossis et coloré de la couleur du curseur du sujet concerné.
				...
				\begin{myfigure}
					...
					\begin{myps}(0,0)(11,8.5)
						...
						\ncline[linewidth=10pt,linecolor=myblue,arrowsize=2pt 2,nodesepA=4pt]{C->}{deformed-residue}{ghost-residue}
					\end{myps}
					\mycaption[fig-pro-exp2-IllustrationDesRendusPourLAffichageDeLaMolecule]{Illustration des rendus pour l'affichage de la molécule}
				\end{myfigure}
			\end{mysubsection}
		\end{mysection}
		\begin{mysection}[sec-pro-exp3-SpecificitesDeLaPremiereExperimentation]{Spécificités de la troisième expérimentation}
			\begin{mysubsection}[sse-pro-exp3-Materiel]{Matériel}
				La troisième expérimentation se focalise sur le travail de groupe et en particulier, sur des \myglos*{glo-Quadrinome}.
				Ce sont donc deux outils de déformation supplémentaires qui sont ajoutés à la plate-forme de base \myref*{sec-pro-MaterielExperimental}.
				...
				La \myref{fig-pro-exp3-SchemaDuDispositifExperimental} illustre le dispositif expérimental par un schéma.
				La \myref{fig-pro-exp3-PhotographieDuDispositifExperimental} est une photographie de la salle d'expérimentation.

				\begin{myfigure}
					\myimage{exp3-schema}
					\mycaption[fig-pro-exp3-SchemaDuDispositifExperimental]{Schéma du dispositif expérimental}
				\end{myfigure}
				\begin{myfigure}
					\myimage{exp3-photo}
					\mycaption[fig-pro-exp3-PhotographieDuDispositifExperimental]{Photographie du dispositif expérimental}
				\end{myfigure}
			\end{mysubsection}
			\begin{mysubsection}[sse-pro-exp3-Visualisation]{Visualisation}
				...
				\end{myfigure}
			\end{mysubsection}
			\begin{mysubsection}[sse-pro-exp3-OutilsDeManipulation]{Outils de manipulation}
				Cette expérimentation fait intervenir quatre sujets.
				Cependant, c'est la prmeière expérimentation pour laquelle aucun outil d'orientation de la molécule n'est fourni aux sujets.
				...
				Durant les précédentes expérimentations, les \myglos*{glo-ConflitDeCoordination} était relativement limité car ne concernait que des \myglos*{glo-Binome}.
				Dans le cadre d'un \myglos{glo-Quadrinome}, un tel outil pourrait produire beaucoup plus de chaos ce que nous ne souhaitons pas.

				En ce qui concerne les outils de déformation, ce sont exactement les mêmes que dans la seconde expérimentation \myref*{sse-pro-exp2-OutilsDeManipulation}.
				Chaque \myglos{glo-Residu} qu'un sujet sélectionne est mis en surbrillance à la fois sur la molécule déformable et sur la molécule dans son état stable.
			\end{mysubsection}
		\end{mysection}
		\begin{mysection}[sec-pro-exp4-SpecificitesDeLaPremiereExperimentation]{Spécificités de la quatrième expérimentation}
			\begin{mysubsection}[sse-pro-exp4-Materiel]{Matériel}
				...
				Deux sujets auront toujours à leur disposition deux outils de déformation adaptés pour le processus de désignation \myref*{sse-pro-exp4-OutilsDeManipulation}.
				...
				La \myref{fig-pro-exp4-SchemaDuDispositifExperimental} illustre le dispositif expérimental par un schéma.
				La \myref{fig-pro-exp4-PhotographieDuDispositifExperimental} est une photographie de la salle d'expérimentation.

				\begin{myfigure}
					\myimage{exp4-schema}
					\mycaption[fig-pro-exp4-SchemaDuDispositifExperimental]{Schéma du dispositif expérimental}
				\end{myfigure}
				\begin{myTodo}{Images à compléter}{Il va falloir faire des photos du dispositif expérimental}
					\begin{myfigure}
						%\myimage{exp4-photo}
						\mycaption[fig-pro-exp4-PhotographieDuDispositifExperimental]{Photographie du dispositif expérimental}
					\end{myfigure}
				\end{myTodo}
			\end{mysubsection}
			\begin{mysubsection}[sse-pro-exp4-Visualisation]{Visualisation}
				...
				\end{myfigure}
			\end{mysubsection}
			\begin{mysubsection}[sse-pro-exp4-OutilsDeManipulation]{Outils de manipulation}
			\end{mysubsection}
		\end{mysection}
	\end{mychapter}
	\begin{mychapter}[cha-pro-MethodeExperimentale]{Méthode expérimentale}
		\begin{mysection}[sec-pro-exp1-PremiereExperimentation]{Première expérimentation}
		\end{mysection}
		\begin{mysection}[sec-pro-exp2-PremiereExperimentation]{Seconde expérimentation}
		\end{mysection}
		\begin{mysection}[sec-pro-exp3-PremiereExperimentation]{Troisième expérimentation}
		\end{mysection}
		\begin{mysection}[sec-pro-exp4-PremiereExperimentation]{Quatrième expérimentation}
		\end{mysection}
	\end{mychapter}
