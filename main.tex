	...
	\begin{mypart}[prt-EtudeDuTravailCollaboratif]{Étude du travail collaboratif}
		...
		\begin{mychapter}[cha-LesDynamiquesDeGroupe]{Les dynamiques de groupe}
			...
			\begin{mysection}[sec-exp3-Resultats]{Résultats}
				...
				\end{mysubsection}
				\begin{mysubsection}[sse-exp3-DefinitionDUnLeader]{Définition d'un leader}
					Cette section va définir les caractéristiques d'un \myemph{leader}.
					Nous utiliserons les données d'un groupe pour alimenter notre propos.
					...
					Le \myemph{leader} est celui qui va diriger les opérations.
					...
					En effet, nos groupes sont des \myemph{structures informelles} dans lesquelles aucun rôle n'est prédéfini.
					Le \myemph{leader} est donc choisi et accepté par les autres sujets.
					Ceci permet une meilleure collaboration des sujets avec une vraie \myemph{motivation sociale}.

					La \myref{fig-exp3-VitesseMoyenneDesEffecteursTerminauxPourChacunDesSujetsDeG1} et la \myref{fig-exp3-ProfilDeForceDuGroupeG1SurLaMoleculePrion-ProfilDeForceDeS1} nous permet de déterminer la stratégie de travail du \myemph{leader}.
					En effet, on constate un grand nombre de sélections ainsi qu'une vitesse élevée.
					Le \myemph{leader} explore l'environnement pour prendre des décisions.
					...
					Par opposition, les \myemph{suiveurs} n'explorent pas l'environnement.
					...
					Le \myemph{suiveur} accepte un ordre du \myemph{leader} et effectue la déformation jusqu'à atteindre l'objectif fixé.
					...
					Cependant, étant donné que toute l'attention est portée sur la déformation, l'effort déployé par les \myemph{suiveurs} pour déformer est plus important.
					Le \myemph{leader} cherchant des tâches à effectuer mais n'effectuant pas totalement les déformation, il ne déploit pas autant d'effort.

					Pour conclure cette section, le \myemph{leader} a un rôle crucial dans la dynamique du groupe.
					...
					L'identification des rôles est nécessaire pour obtenir de bonne performances et éviter le phénomène de \myemph{paresse sociale} \myref*{sse-exp3-AvantageDuRemueMeningesPourLesQuadrinomes}.
					Cependant, le \myemph{leader} doit pouvoir se faire accepter des autres membres du groupes et ainsi garder une bonne cohésion.
				\end{mysubsection}
			\end{mysection}
