		...
		\begin{mysection}[sec-exp4-Resultats]{Résultats}
			...
			\begin{mysubsection}[sse-exp4-AmeliorationDesPerformances]{Amélioration des performances}
				\begin{mysubsubsection}[sss-exp4-AmeliorationDesPerformances-DonneesEtTestsStatistiques]{Données et tests statistiques}
					...
					\begin{myfigure}
						\psset{xunit=0.230769231\textwidth,yunit=30cm}
						\begin{myps}(-0.6,-0.04)(2,0.155)
							\myaxes(0,2){haptique}(0,0.150)[0.025]{sélections~(nb/s)}
							\myboxplot{exp4-button1-haptic.csv}
						\end{myps}
						\mycaption[fig-exp4-NombreDeSelectionsParSecondeEffectueesParLesOperateursPourLaDeformationAvecEtSansHaptique]{Nombre de sélections par seconde effectuées par les opérateurs pour la déformation avec et sans haptique}
					\end{myfigure}

					La \myref{fig-exp4-NombreDeSelectionsParSecondeEffectueesParLesOperateursPourLaDeformationAvecEtSansHaptique} présente le nombre de sélections par seconde effectuées par les opérateurs pour une déformation \myvard{3} avec et sans haptique \myvari{1}.
					L'analyse montre un effet significatif de la présence d'une assistance haptique \myvari{1} sur le nombre de sélections \myvard{3} (\myanova{exp4-button1-haptic-anova.tex}).
				...
				\begin{mysubsubsection}[sss-exp4-AmeliorationDesPerformances-AnalyseEtDiscussion]{Analyse et discussion}
					...
					D'ailleurs, ni le score \myacro{acr-RMSD}, ni le temps pour atteindre ce score n'est amélioré avec les outils d'assistance haptique à la communication proposés \myref*{fig-exp4-TempsPourAtteindreLeScoreRMSDMinimumAvecEtSansHaptique}.

					Pourtant, si on effectue une analyse détaillée en fonction des tâches proposées, on constate que sur le complexe de molécules \myNusENusG, les outils d'assistance permettent d'améliorer significativement les performances.
					En ce qui concerne le scénario~\myscenario{1} (molécule \myUbiquitin), aucune évolution n'est observée.

					Le complexe de molécules \myNusENusG (scénario~\myscenario{2}) reste clairement la tâche la plus complexe à réaliser part le nombre de \myglos*{glo-Residu} à déformer et part la taille globale des deux molécules en présence.
					D'ailleurs, les annotations vidéos ont permis de constater à plusieurs reprises une sorte d'abandon face à la complexité de cette tâche.
					Cependant, il semble que la présence d'outils d'assistance à la communication ait permis d'améliorer significativement les performances globales.

					On observe également une baisse significative de la fréquence du nombre de déformations effectuées par les opérateurs \myref*{fig-exp4-NombreDeSelectionsParSecondeEffectueesParLesOperateursPourLaDeformationAvecEtSansHaptique}.
					Le coordinateur est le seul des sujets à ne pas être occupé à déformer la molécule.
					Il se consacre à évaluer les meilleurs déformations avec une vue globale de la tâche.
					Avec les outils d'assistance haptique à sa disposition, il peut communiquer plus facilement et plus rapidement ces directives.
					Ceci a pour effet que les opérateurs effectuent moins d'actions solitaires qui peuvent être brèves car inutiles ou interrompue par une requête du coordinateur.
					Les séquence de sélection et donc de déformation sont alors plus longues et moins nombreuses.
