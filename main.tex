...
\begin{document}
	...
	\begin{mychapter}[cha-sota-EtudeBibliographique]{Étude bibliographique}
		...
		\begin{mysection}[sec-sota-ContexteDeTravail-LaModelisationMoleculaire]{Contexte de travail : la modélisation moléculaire}
			...
			Dans cette section, nous commencerons par présenter le \myglos{glo-DockingMoleculaire} avant de montrer les différentes solutions existantes pour traiter ce problème complexe.
			\begin{mysubsection}[sse-sota-LeDockingMoleculaire]{Le \myglosnl{glo-DockingMoleculaire}}
				Dans le domaine de la modélisation moléculaire, le \myglos{glo-DockingMoleculaire} à prédire l'orientation optimale entre deux molécules afin de créer un complexe de molécules stable \myref*{fig-sota-ComplexeDeMoleculesAssembleAPartirDeDeuxMolecules}.
				\mycite[author]{Fischer-1894} proposent une image illustrant le \myglos{glo-DockingMoleculaire} avec son modèle \og clef-serrure \fg décrit de la façon suivante.
				...
				La métaphore s'arrête ici car il est nécessaire d'évaluer la conformation entre les deux molécules et l'ouverture d'une serrure à l'aide d'une clef ne saurait suffire à expliquer l'évaluation d'un \myglos{glo-DockingMoleculaire}.
				La stabilité d'un complexe de molécules est évaluée sur deux mesures distinctes : la complémentarité géométrique et la complémentarité chimique \myref*{fig-sota-MesuresPourLEvaluationDuDockingMoleculaire}.
				L'importance de la complémentarité géométrique, aussi appelée complémentarité structurale \mycite{Church-1977}, a été mise en valeur par \mycite[author]{Jiang-2003}; elle consiste à trouver une partie du profil sur chaque molécule afin de pouvoir les emboîter, comme dans un puzzle.
				...
				La complémentarité chimique s'intéresse plus précisément aux interactions chimiques entre les molécules comme les charges électrostatiques \mycite{McCoy-1997}, les ponts hydrogènes \mycite{Arunan-2011} ou encore les régions hydrophiles et hydrophobes \mycite{Blalock-1984}.
				...
				\end{myfigure}
			\end{mysubsection}
			\begin{mysubsection}[sse-sota-RealisationDUnDockingMoleculaire]{Réalisation d'un \myglosnl{glo-DockingMoleculaire}}
				La réalisation d'un \myglos{glo-DockingMoleculaire} nécessite de trouver les sites de liaison entre les molécules basés sur la complémentarité (géométrique et/ou chimique).
				\mycite[author]{Schulz-Gasch-2004} ou encore \mycite[author]{Leach-2006} exposent un état de l'art sur les moyens d'évaluer un \myglos{glo-DockingMoleculaire}.
				En général, les algorithmes de recherche commencent par identifier les différents sites de liaisons potentiels exclusivement par complémentarité géométrique.
				Cette première évaluation permet de filtrer de nombreuses possibilités.
				Puis une évaluation chimique partielle ou complète est éventuellement intégré au moteur de recherche.

				La combinatoire de ce problème est tellement importante qu'une approche exhaustive est impossible à réaliser dans un temps raisonnable.
				Afin de trouver ces sites de liaison, différents types d'algorithmes ont été proposés.
				\mycite[author]{Jones-1997} s'orientent vers des algorithmes génétiques avec le système \myGOLD pour évaluer la complémentarité géométrique et complète la recherche avec une analyse des champs de forces.
				Puis, toujours basé sur les algorithmes génétiques, une amélioration est proposée par \mycite[author]{Morris-1998} avec le système \myAutoDock, probablement la solution la plus utilisée actuellement \mycite{Grosdidier-2007}.
				Les méthodes basées sur la méthode de \myname{Monte-Carlo} sont également étudiées avec les systèmes \myGlide \mycite{Friesner-2004,Halgren-2004} ou encore \myBoxSearch \mycite{Hart-1992,Cummings-1995}.
				Enfin, l'outil \myFlexX \mycite{Rarey-1997,Rarey-1999}, également très utilisé, tente de reconstruire le ligand de manière incrémentale.

				Les solutions sont nombreuses mais devant la complexité du problème, aucune de ces solutions ne semble efficace pour tous les complexes de molécules testées.
				Pourtant, les solutions présentées ci-dessus utilisent déjà une approximation de l'environnement moléculaire.
				En effet, ces solutions considèrent les molécules comme des corps rigides hors une molécule est constituée d'un ensemble d'atomes possédant chacun une mobilité par rapport à ces voisins : une molécule est un corps flexible.
				La \myref{fig-sota-IllustrationDUneMoleculeAvecCesHelicesAlphaEtCesFeuilletsBeta} représente un complexe de molécules ainsi que sa structure interne constituée d'\myhelice* et de \myfeuillet*.
				Les \myhelice* et les \myfeuillet* constituent la plus grosse granularité de la flexibilité d'une molécule alors que les atomes constituent la granularité la plus fine.
				...
				Afin de répondre à cette problématique de flexibilité, \mycite[author]{Osterberg-2002} proposent une extension à \myAutoDock en considérant le complexe de molécules sous forme de grille pour grossier la granularité et augmenter la vitesse d'évaluation d'une configuration de \myglos{glo-DockingMoleculaire}.
