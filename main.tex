	...
	\begin{mychapter}[cha-Shaddock-ManipulationCollaborativeDeMolecules]{\myShaddock\ -- Manipulation collaborative de molécules}
		...
		\begin{mysection}[sec-Shaddock-LesOutilsDInteraction]{Les outils d'interaction}
			\begin{mysubsection}[sse-Shaddock-LesOutilsExistants]{Les outils existants}
				...
				Les outils proposés par défaut dans \myacro{acr-VMD} ont été utilisés dans la première expérimentation \myref*{cha-RechercheCollaborativeDansUnEnvironnementVirtuelComplexe}.
		...
		\end{mysection}
	\end{mychapter}
	\begin{mychapter}[cha-RechercheCollaborativeDansUnEnvironnementVirtuelComplexe]{Recherche collaborative dans un environnement virtuel complexe}
		\begin{mysection}[sec-exp1-Introduction]{Introduction}
			Cette introduction nous a permis d'identifier les principales tâches élémentaires concernant l'interaction en environnement virtuel, décrites par \mycite[author]{Bowman-1999} : les \myacro{acr-PCV} \mycite{Fuchs-2006a}.
			...
			Cependant, la sélection au sein d'une simulation moléculaire est une problématique complexe à part entière \mycite{Delalande-2010}, notamment à cause de la complexité des molécules liée au nombre important d'atomes et à leur structure repliée.
			...
			Le prétexte à cette étude sera une expérimentation sur la recherche de \myglos*{glo-Residu} présentée dans la \myref{sec-exp1-RechercheDeResiduAuSeinDUneMolecule}.
			L'étude proposée permettra de montrer l'intérêt du travail collaboratif sur des tâches de nature complexes.
			Cependant, nous verrons que les différents \myglos*{glo-Binome} adoptent des stratégies relativement différentes, menant à des performances hétérogènes.
		...
		\begin{mysection}[sec-exp1-RechercheEtSelectionCollaborative]{Recherche et sélection collaborative}
			\begin{mysubsection}[sse-exp1-TravauxExistants]{Travaux existants}
				Les quatre \myacro*{acr-PCV} décrites par \mycite[author]{Fuchs-2006a} sont une succession d'étapes nécessaires de l'interaction avec un environnement virtuel.
				Dans ce chapitre, nous proposons d'étudier les deux premières de ces quatre tâches élémentaires \myref*{sse-LAmarrageMoleculaire}: la \myemph{recherche} et la \myemph{sélection}.
				Ces étapes sont cruciales car elles ont un impact important sur les étapes suivantes que sont la \myemph{manipulation} et l'\myemph{évaluation}.
				...
				Pourtant, elle est déjà relativement développée pour les \myacro*{acr-AFM} mais les contraintes techniques de cette technique sont très éloignées de nos contraintes techniques en environnement virtuel donc nous ne nous étendrons pas sur ce sujet.
				La taille et la complexité des molécules nécessite des solutions de sélections adaptées.
				Les logiciels les plus utilisés comme \myPyMOL et \myacro{acr-VMD} proposent des moteurs de sélection à base de chaîne de caractères.
...
vmd> set mysel [atomselect "top" "name C or name O or name N or name CA"]
				...
				Dans le cadre de l'exploration virtuelle d'une molécule, \myacro{acr-VMD} propose par exemple une sélection par \myemph{picking}\footnote{Méthode qui consiste à trouver l'élément pointé par la souris (\myTwoD) dans l'environnement virtuel (\myThreeD).} à l'aide de la souris.
				Cependant, ce type de méthode atteint rapidement ces limites en terme de précision dès que la molécule est de taille trop importante : la complexité de la molécule sur la dimension de la profondeur rend relativement inutilisable la méthode du \myemph{picking}.
				...
				Cependant, les travaux ont évolué vers des corps flexibles avec \mycite[author]{Delalande-2010} qui utilise les périphériques haptiques pour aider à la localisation de ponts ioniques au sein de simulation moléculaire en temps-réel.
			...
			\begin{mysubsection}[sse-exp1-Objectifs]{Objectifs}
				Dans ce chapitre, nous aborderons les tâches de recherche et de sélection dans un \myacro{acr-EVC}.
			...
			\end{mysubsection}
		\end{mysection}
		\begin{mysection}[sec-exp1-RechercheDeResiduAuSeinDUneMolecule]{Recherche de résidu au sein d'une molécule}
			Afin de répondre à nos hypothèses de travail, résumée dans la \myref{sse-met-exp1-Hypotheses}, nous mettons en place une expérimentation de recherche de \myglos{glo-Residu} au sein d'une molécule pendant une simulation moléculaire en temps-réel.
			...
			\begin{mysubsection}[sse-exp1-DescriptionDeLaTache]{Description de la tâche}
				La tâche proposée consiste à trouver puis à extraire des \myglos*{glo-Residu} au sein d'une molécule.
			...
			\begin{mysubsection}[sse-exp1-SpecificitesDuProtocoleExperimental]{Spécificités du protocole expérimental}
				...
				\begin{mysubsubsection}[sss-exp1-Visualisation]{Visualisation}
					...
					Une représentation de cette molécule durant la première expérimentation est sur la \myref{fig-exp1-RepresentationDeLaMoleculePrionPourLExperimentation}.
	...
	\begin{mychapter}[cha-DeformationCollaborativeDeMolecule]{Déformation collaborative de molécule}
		...
		\begin{mysection}[sec-exp2-Contexte]{Contexte}
			...
			La précédente expérimentation \myref*{cha-RechercheCollaborativeDansUnEnvironnementVirtuelComplexe} a souligné l'avantage de la collaboration sur des tâches nécessitant un couplage fort.
		...
		\begin{mysection}[sec-exp2-ExperimentationDeDeformationDeMolecule]{Expérimentation de déformation de molécule}
			...
			\begin{mysubsection}[sse-exp2-SpecificitesDuProtocoleExperimental]{Spécificités du protocole expérimental}
				...
				\begin{mysubsubsection}[sss-exp2-Visualisation]{Visualisation}
					...
					La \myref{fig-exp2-RepresentationDeLaMoleculeTRPZIPPERPourLeScenario1A} représente le scénario~\myscenario{1A}, la \myref{fig-exp2-RepresentationDeLaMoleculeTRPCAGEPourLeScenario1B} représente le scénario~\myscenario{1B}, la \myref{fig-exp2-RepresentationDeLaMoleculeTRPZIPPERPourLeScenario2A} représente le scénario~\myscenario{2A} et la \myref{fig-exp2-RepresentationDeLaMoleculeTRPCAGEPourLeScenario2B} représente le scénario~\myscenario{2B}.
		...
		\begin{mysection}[sec-exp3-ExperimentationSurLesDynamiquesDeGroupe]{Expérimentation sur les dynamiques de groupe}
			...
			\begin{mysubsection}[sse-exp3-SpecificitesDuProtocoleExperimental]{Spécificités du protocole expérimental}
				...
				\begin{mysubsubsection}[sss-exp3-Visualisation]{Visualisation}
					...
					La molécule \myPrion est utilisée pour le scénario~\myscenario{1} sur la \ref{fig-exp3-RepresentationDeLaMoleculePrionPourLeScenario1} et la molécule \myUbiquitin est utilisée pour le scénario~\myscenario{2} sur la \ref{fig-exp3-RepresentationDeLaMoleculeUbiquitinPourLeScenario2}.
		...
		\begin{mysection}[sec-exp3-Resultats]{Résultats}
			\begin{mysubsection}[sse-exp3-EvolutionDesPerformances]{Évolution des performances}
				...
				\begin{mysubsubsection}[sss-exp3-EvolutionDesPerformances-AnalyseEtDiscussion]{Analyse et discussion}
					...
					L'étude précédente présentée dans le \myref{cha-RechercheCollaborativeDansUnEnvironnementVirtuelComplexe} a montré que le travail en configuration \myglos*{glo-Bimanuel} permet de moins bonnes performances qu'un travail collaboratif en configuration \myglos*{glo-Monomanuel}.
		...
		\begin{mysection}[sec-met-exp4-PremiereExperimentation]{Quatrième expérimentation}
			...
			\begin{mysubsection}[sse-met-exp4-Procedure]{Procédure}
				...
				\begin{myparagraph}[par-met-exp4-Phase5-PremiereEtapeDEvaluation]{Phase~\mynum{5} : première étape d'évaluation}
					...
					Au début de chaque scénario, une période de \mynum[mn]{1} de remue-méninges est laissée aux sujets pendant laquelle ils peuvent visualiser et explorer la molécule non soumise à la simulation.
