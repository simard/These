...
vmd> set mysel [atomselect "top" "name C or name O or name N or name CA"]
	...
	\begin{mychapter}[cha-LesDynamiquesDeGroupe]{Les dynamiques de groupe}
		\begin{mysection}[sec-exp3-Introduction]{Introduction}
			...
			À cette époque, seul le travail coopératif est étudié, le travail collaboratif n'est étudié que quelques années plus tard.
			\mycite[author]{Bales-1950} s'intéresse particulièrement à la communication dans les petits groupes également appelés groupes restreints.
			L'ensemble de ces études concernent des collaborations asynchrones ou, le cas échéant, des collaborations pour des tâches à faible couplage.

			Ce chapitre constitue notre première étude sur le travail collaboratif concernant des groupes d'utilisateurs\footnote{On considère qu'un groupe est constitué au minimum de trois personnes \mycite{Roethlisberger-1939}.}.
			...
			Nous souhaitons tester la mise en place de périodes de \mybrainstorming\footnote{Pour la suite des développements, le mot \mybrainstorming sera utilisé plutôt que le mot \myemph{remue-méninges} car il est plus utilisé dans la littérature.} afin de catalyser le potentiel du groupe.
