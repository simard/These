...
\begin{document}
	...
	\begin{mychapter}[cha-RechercheCollaborativeDeResiduSurUneMolecule]{Recherche collaborative de résidu sur une molécule}
		\begin{mysection}[sec-exp1-Introduction]{Introduction}
			Le premier chapitre nous a permis d'identifier les principales tâches élémentaires concernant l'interaction en environnement virtuel, décrites par \mycite[author]{Bowman-1999} : les \myacro*{acr-PCV} \mycite{Fuchs-2006a}.
			...
			Cependant, la sélection au sein d'une simulation moléculaire est une problématique complexe à part entière \mycite{Delalande-2010}, notamment à cause de la complexité des molécules liée au nombre important d'atomes et à la structure repliée.
			...
			Le prétexte à cette étude sera une expérimentation sur la recherche de \myglos*{glo-Residu} présentée dans la \myref{sec-exp1-PresentationDeLExperimentation}.
			...
			Nous identifierons ainsi les avantages de la collaboration mais également les limites d'une telle configuration de travail.
		\end{mysection}
...
vmd> set mysel [atomselect "top" "name C or name O or name N or name CA"]
		...
		\begin{mysection}[sec-exp1-PresentationDeLExperimentation]{Présentation de l'expérimentation}
			Afin de répondre à nos hypothèses de travail, résumées dans la \myref{sse-met-exp1-Hypotheses}, nous mettons en place une expérimentation de recherche de \myglos{glo-Residu} au sein d'une molécule pendant une simulation moléculaire en temps-réel.
			...
			\begin{mysubsection}[sse-exp1-DescriptionDeLaTache]{Description de la tâche}
				...
				Les \myglos*{glo-Residu} sont des groupes d'atomes s'associant les uns aux autres pour former une molécule.
			...
			\begin{mysubsection}[sse-exp1-SpecificitesDuProtocoleExperimental]{Spécificités du protocole expérimental}
				...
				\begin{mysubsubsection}[sss-exp1-Materiel]{Matériel}
					...
					Le \myglos{glo-Residu} cible doit donc être affiché aux sujets pendant toute la durée de la taâche.
	...
	\begin{mychapter}[cha-DeformationCollaborativeDeMolecule]{Déformation collaborative de molécule}
		\begin{mysection}[sec-exp2-Introduction]{Introduction}
			Cette première expérimentation nous a permis d'étudier les premières \myacro*{acr-PCV} que sont la \myemph{recherche} et la \myemph{sélection}.
			Afin de notre compléter notre étude du travail collaboratif dans un environnement virtuel complexe, nous souhaitons à présent nous intéresser à la \myacro{acr-PCV} suivante : la \myemph{déformation}.
			En effet, on trouve déjà plusieurs environnements virtuels permettant de manipuler des molécules rigides afin d'effectuer de l'\myglos{glo-AmarrageMoleculaire} comme les travaux de \mycite[author]{Levine-1997} ou encore de \mycite[author]{Ferey-2008a}.
			Cependant, l'amarrage moléculaire s'effectue de manière optimale lorsqu'on a la possibilité de déformer les molécules.
			Ceci est rendu possible par l'avénement des simulations interactives en temps-réel, notamment avec \myacro{acr-IMD} développé par \mycite[author]{Stadler-1997}.
			Plus récemment, \mycite[author]{Delalande-2009} ont également amené une pierre à l'édifice avec \myMDDriver puis ensuite avec l'utilisation d'interface haptique pour la manipulation et la déformation \mycite{Delalande-2010}.

			Dans ce chapitre, nous allons étudier le processus de déformation d'une molécule.
			Nous souhaitons vérifier la pertinence de la configuration de travail collaboratif pour appréhender la déformation d'une molécule.
			De plus, nous aborderons la question de l'apprentissage au sein d'un groupe.
			En effet, certains éléments de la première expérimentation semble indiquer qu'une configuration collaborative stimule l'apprentissage concernant la plate-forme, les outils ou encore la tâche à réaliser.
		\end{mysection}
		\begin{mysection}[sec-exp2-DeformationCollaborativeEnEnvironnementVirtuel]{Déformation collaborative en environnement virtuel}
			\begin{mysubsection}[sse-exp2-TravauxExistants]{Travaux existants}
				...
				\mycite[author]{Shen-2006} propose déjà une solution pour déformer des objets non-rigides à l'aide de retour haptique.
				Les objets concernés sont des faible complexité, comme des sphères par exemple.
				Puis, \mycite[author]{Peterlik-2009} effectue une thèse sur les déformations de tissus cellulaires.
				Là encore, les éléments déformables sont de faibles complexité et n'ont quasiment pas d'application concrète.

				Cependant, afin d'effectuer des déformations plus pertinentes, certains se sont intéressés aux processus de déformation collaboratifs dans les \myacro*{acr-EVC}.
				\mycite[author]{Sumengen-2007} propose une plate-forme permettant la déformation de maillages pouvant être utilisé pour des simulations d'objets déformables (tissus, organes, \myetc) dans un \myacro{acr-EVC}.
				...
				De son côté, \mycite[author]{Tang-2010a} propose une plate-forme client/serveur de déformation collaborative de maillages.
				...
				\mycite[author]{Muller-2006} développe le logiciel \myClayWorks, repris plus tard par \mycite[author]{Gorlatch-2009}, permettant la sculpture virtuelle sur glaise.
				...
				En effet, tous les \myacro*{acr-EVC} proposés sont consacrés à la collaboration distante (bien que les travaux de \mycite[author]{Sumengen-2007} n'aient été testés que sur une réseau interne) et la problématique d'un échange de données en temps-réel est l'une des principales problématiques traitée.
			\end{mysubsection}
			\begin{mysubsection}[sse-exp2-Objectifs]{Objectifs}
				...
				La comparaison opposera des \myglos*{glo-Monome} en configuration \myglos*{glo-Bimanuel} à des \myglos*{glo-Binome} en configuration \myglos*{glo-Monomanuel}.
				...
				Nous supposons que les \myglos*{glo-Binome} en configuration \myglos*{glo-Monomanuel} soient plus performants que les \myglos*{glo-Monome} en configuration \myglos*{glo-Bimanuel}.
				...
				En effet, les tâches complexes fournissent une charge cognitive de travail très importante; plus cette charge de travail est importante et plus les \myglos*{glo-Monome} risquent de ne pas être capables d'appréhender cette charge.
				Notre hypothèse estime que les tâches les plus complexes seront quasiment inaccessibles aux \myglos*{glo-Monome}.
				...
				Nous supposons que la motivation et l'échange qui naît d'un travail en groupe va permettre aux \myglos*{glo-Binome} d'appréhender plus rapidement la plate-forme, les outils ou encore la tâche.
			\end{mysubsection}
		\end{mysection}
				...
				\begin{mysubsubsection}[sss-exp2-DescriptionDesScenarios]{Description des scénarios}
					...
					\begin{description}
						...
						\item[Scénario~\myscenario{2b}]
							...
							La manipulation coordonnée de deux \myglos*{glo-Residu} est la seule solution pour atteindre l'objectif.
					\end{description}
		...
		\begin{mysection}[sec-exp2-Resultats]{Résultats}
			...
			\begin{mysubsection}[sse-exp2-GainDePerformanceSurLesTachesComplexes]{Gain de performance sur les tâches complexes}
				...
				Dans la section précédente, nous avons montré que la configuration \myglos*{glo-Bimanuel} ne permet pas d'égaler les performances d'un travail collaboratif.
	...
	\begin{mychapter}[cha-LesDynamiquesDeGroupe]{Les dynamiques de groupe}
		\begin{mysection}[sec-exp3-Introduction]{Introduction}
			...
			Les objectifs de cette nouvelle expérimentation sont exposés dans la \myref{sec-exp3-CollaborationDeGroupe} accompagnés par les hypothèses.
			La tâche et les modifications apportées à la plate-forme correspondantes sont détaillées dans la \myref{sec-exp3-PresentationDeLExperimentation}.
		...
		\begin{mysection}[sec-exp3-Resultats]{Résultats}
			\begin{mysubsection}[sse-exp3-EvolutionDesPerformances]{Évolution des performances}
				...
				\begin{mysubsubsection}[sss-exp3-EvolutionDesPerformances-AnalyseEtDiscussion]{Analyse et discussion}
					...
					L'étude précédente présentée dans le \myref{cha-RechercheCollaborativeDeResiduSurUneMolecule} a montré que le travail en configuration \myglos*{glo-Bimanuel} permet de moins bonnes performances qu'un travail collaboratif en configuration \myglos*{glo-Monomanuel}.
