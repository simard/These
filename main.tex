	...
	\begin{mypart}[prt-Introduction]{Introduction}
		...
		\begin{mychapter}[app-ShaddockSystemeCollaboratifPourLaManipulationDeMolecules]{\myShaddock\ -- Système collaboratif pour la manipulation de molécules}
			\begin{mysection}[sec-PlateFormesCollaborativesExistantes]{Plates-formes collaboratives existantes}
				Le travail collaboratif est un centre d'intérêt qui se développe de plus en plus vite.
				\myShaddock est le nom de la plate-forme développée dans le cadre de cette thèse.
				Les choix d'architectures sont exposés dans cette section.
				...
				\begin{mysubsection}[sse-ArchitectureDuSysteme]{Architecture du système}
					Deux types d'architecture sont possibles pour les \myacro{acr-EVC} : pair-à-pair ou client/serveur.
					Parmi les architectures pair-à-pair, on peut citer les travaux récents sur l'haptique de \mycite[author]{Iglesias-2008} sur les assemblages collaboratifs ou \mycite[author]{Kim-2004} sur le déplacement collaboratif d'une boîte.
					
					Pour les architectures client/serveur, \mycite[author]{Norman-2010} s'intéresse particulièrement aux influences du réseau sur les interactions visuo-haptiques.
					\mycite[author]{Huang-2010} propose un système haptique client/serveur pour la manipulation d'un jeu de construction par blocs.

					\mycite[author]{Marsh-2006} propose une comparaison de ces deux types d'architecture et en vient à la conclusion que l'architecture pair-à-pair est la plus performante en terme de latence.
					Cependant, elle est adaptée pour les environnements statiques ou faiblement dynamiques.
					En effet, des latences ou des erreurs de transmission sur le réseau vont introduire des erreurs de synchronisation.
					Avec des erreurs de synchronisation, les environnements fortement dynamiques vont diverger rapidement et provoquer des incohérences pour tous les acteurs.
					L'architecture client/serveur, bien que moins performante, permettra de conserver une cohérence pour tous les acteurs.

					Dans le cadre d'une architecture client/serveur, les simulation seront effectuées sur un serveur et les données de simulation seront transmises à tous les acteurs de l'\myacro{acr-EVC}.
					Ainsi, chaque utilisateur pourra bénéficier d'une visualisation particulière et d'outils d'interaction personnels.

					La plate-forme \myShaddock présentée ici est une architecture client/serveur.
				\end{mysubsection}
				\begin{mysubsection}[sse-VuePartagee]{Vue partagée}
					Dans un \myacro{acr-EVC}, un point important est la disponibilité de la vue.
					Elle peut être soit privée pour chaque utilisateur, soit partagée.

					Les nombreux travaux sur la télé-présence rende la littérature très prolifique concernant les vues privées.
					Les premiers travaux sur la collaboration haptique sont fournis par \mycite[author]{Basdogan-2000} dans lesquels il place deux utilisateurs dans deux pièces différentes pour effectuer une tâche nécessitant de la synchronisation.
					...
					\mycite[author]{Sallnas-2010} a beaucoup travaillé sur l'apport de l'haptique sur la qualité de la conscience des autres.
					Elle montre à plusieurs reprises l'intérêt d'être conscients des faits et gestes des autres utilisateurs.
					\mycite[author]{Casera-2006} privilégie une vue publique pour améliorer la conscience des autres utilisateurs.
					\mycite[author]{Tang-2006} a exploré la collaboration sur une table tactile avec une vue partagée.

					Étant donné que \myShaddock est prévue pour la collaboration sur des tâches complexes, le travail à distance est une contrainte.
					...
					Ils ont ainsi la possibilité de communiquer le plus librement et le plus naturellement possible.
			...
			\begin{mysection}[sec-InteractionAvecLaSimulationMoleculaire]{Interaction avec la simulation moléculaire}
				La plate-forme \myShaddock doit également intégrer un moyen d'interagir avec cette simulation en temps-réel.
				Les besoins particuliers de cette interaction seront présentés dans la \myref{sse-interaction-LesBesoins} puis les solutions logicielles dans la \myref{sse-interaction-InteractionParInterfaceHaptique}.
				\begin{mysubsection}[sse-interaction-LesBesoins]{Les besoins}
					Afin de pouvoir modifier la simulation en temps-réel, il faut pouvoir agir en manipulant des éléments de la molécule.
					Les atomes de la molécule doivent pouvoir être manipuler en leur appliquant des efforts.
