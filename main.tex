	...
	\begin{mychapter}[cha-sota-EtudeBibliographique]{Étude bibliographique}
		...
		\begin{mysection}[sec-sota-ContexteDeTravail-LaModelisationMoleculaire]{Contexte de travail : la modélisation moléculaire}
			...
			\begin{mysubsection}[sse-sota-LeDockingMoleculaireEnEnvironnementVirtuel]{Le \myglosnl{glo-DockingMoleculaire} en environnement virtuel}
				...
				\mycite[author]{Bergman-1993} proposent la plate-forme \myVIEW avec différents rendus graphiques pour représenter les molécules \myref*{fig-sota-RepresentationAvanceesDeMoleculesProposeeParBergman1993}.
				...
				Puis, la modélisation moléculaire assistée par ordinateur est proposée sur la plate-forme \myHIMM (\myemph{Highly Immersive Molecular Modeling}) \mycite{Drees-1996,Drees-1998}.
				...
				\end{equation}
				Enfin, la manipulation interactive avec des structures moléculaires flexibles devient possible avec la plate-forme proposée par \mycite[author]{Delalande-2009} à laquelle on peut ajouter la possibilité d'interagir avec une interface haptique \mycite{Delalande-2010}.
				...
				Par exemple, \mycite[author]{Sankaranarayanan-2003} proposent une plate-forme interactive avec des interfaces haptiques pour enseigner différents aspects qui constituent la structure moléculaire.
		...
		\begin{mysection}[sec-sota-CollaborationEnEnvironnementVirtuel]{Collaboration en environnement virtuel}
			...
			\begin{mysubsection}[sse-sota-LaCommunicationImpliciteEtExplicite]{La communication implicite et explicite}
				...
				\begin{mysubsubsection}[sss-sota-LInterReferencement]{L'inter-référencement}
					La notion d'inter-référencement est très récente et c'est \mycite[author]{Chastine-2007} qui l'a mise en évidence à travers sa plate-forme \myAMMPVis \mycite{Chastine-2005,Chastine-2006}.
		...
		\end{mysection}
	\end{mychapter}
	\begin{mychapter}[cha-Shaddock-ManipulationCollaborativeDeMolecules]{\myShaddock\ -- Plate-forme de manipulation collaborative de molécules}
		\begin{mysection}[sec-Shaddock-Introduction]{Introduction}
			...
			L'ensemble des éléments de la plate-forme sont organisés selon une architecture client/serveur; les raisons de ce choix sont expliquées dans la \myref{sse-Shaddock-ArchitectureLogicielleAdoptee}.
		...
		\begin{mysection}[sec-Shaddock-ArchitectureMaterielleEtLogicielle]{Architecture matérielle et logicielle}
			...
			\begin{mysubsection}[sse-Shaddock-ArchitectureLogicielleAdoptee]{Architecture logicielle adoptée}
				...
				\begin{mysubsubsection}[sss-Shaddock-ArchitectureClientServeur]{Architecture client/serveur}
					...
					La plate-forme \myShaddock est pourvue d'une architecture client/serveur comme on peut le voir sur la \myref{fig-Shaddock-DiagrammeDeDeploiementUMLDeLaPlateFormeShaddock}.
				\end{mysubsubsection}
		...
		\begin{mysection}[sec-Shaddock-PlateFormeDeSimulationEtDeVisualisation]{Plate-forme de simulation et de visualisation}
			...
			\begin{mysubsection}[sse-Shaddock-ModuleDeVisualisationMoleculaire]{Module de visualisation moléculaire}
				...
				Les quatre représentations différentes \myref*{fig-simulation-IllustrationsDesRendusGraphiquesDeMoleculesSurVMD} sélectionnées sur la plate-forme \myShaddock sont présentées ici :
				\begin{description}
			...
			\begin{mysubsection}[sse-Shaddock-ModuleDeSimulationMoleculaire]{Module de simulation moléculaire}
				...
				C'est pourquoi le moteur de simulation moléculaire \myacro{acr-NAMD} a été retenu pour notre plate-forme.
...
vmd> set mysel [atomselect "top" "name C or name O or name N or name CA"]
		...
		\begin{mysection}[sec-exp1-Resultats]{Résultats}
			...
			\begin{mysubsection}[sse-exp1-ResultatsQualitatifs]{Résultats qualitatifs}
				...
				La seconde partie a pour but d'évaluer la plate-forme\footnote{L'échelle de notation est comprise entre \mynum{1} et \mynum{5} mais les moyennes ont été normalisées entre \mynum{0} et \mynum{4}.}.
	...
	\begin{mychapter}[cha-TravailCollaboratifAssisteParHaptique]{Travail collaboratif assisté par haptique}
		...
		\begin{mysection}[sec-exp4-AssistanceHaptiquePourLaCommunication]{Assistance haptique pour la communication}
			\begin{mysubsection}[sse-exp4-TravauxExistants]{Travaux existants}
				Les travaux à l'origine de communication haptique ont principalement été initiés par les résultats de \mycite[author]{Sallnas-2000} avec une expérimentation sur la manipulation synchrone de cube au sein d'un environnement virtuel.
				...
				Sur le même principe de manipulation synchrone, \mycite[author]{Basdogan-2000} proposent le déplacement d'un anneau autour d'un fil avec pour objectif de ne pas toucher le fil.
				...
				Dans la continuité des travaux de \mycite[author]{Oakley-2001}, \mycite[author]{Moll-2009} effectuent des expérimentations avec des outils haptiques similaires dans un environnement virtuel de construction par bloc en \myThreeD.
				...
				En l'occurence, \mycite[author]{Moll-2009} mettent en évidence l'utilité des retours haptiques pour la désignation de cible en environnement virtuel.

				Le terme \myemph{communication haptique} a pris tout son sens avec les premières tentatives de \myemph{langages haptiques}.
				Tout d'abord, \mycite[author]{Chang-2002} proposent d'augmenter l'information avec un code défini de vibration lors de communication de type \myemph{talkie-walkie}.
				...
				\mycite[author]{Chan-2008} utilise également des mots haptiques pour aider et assister les contrôles d'accès à un système lors de la réalisation d'une tâche collaborative.
				...
				\mycite[author]{Ullah-2011} s'est intéressé plus particulièrement aux influences de l'utilisation de retours haptiques dans la collaboration sur le sentiment de présence et sur la conscience périphérique \myref*{sse-sota-LaCommunicationImpliciteEtExplicite}.
				...
				Puis, une tâche de déplacement d'objet virtuel (une cheville à déplacer dans un trou) que \mycite[author]{Ullah-2010} montre une amélioration significative du sentiment de présence et de la conscience périphérique.
			\end{mysubsection}
			\begin{mysubsection}[sse-exp4-Objectifs]{Objectifs}
				...
				L'haptique est un outil adapté à cette fonctionnalité comme le suggèrent \mycite[author]{Moll-2009}.
		...
		\begin{mysection}[sec-exp4-Resultats]{Résultats}
			...
			\begin{mysubsection}[sse-exp4-AmeliorationDesPerformances]{Amélioration des performances}
				\begin{mysubsubsection}[sss-exp4-AmeliorationDesPerformances-DonneesEtTestsStatistiques]{Données et tests statistiques}
					...
					La \myref{fig-exp4-ScoreRMSDMinimumAtteintAvecEtSansHaptique} présente le score \myglos{acr-RMSD} minimum atteint \myvard{1} avec et sans haptique \myvari{1}.
					L'analyse montre qu'il n'y a pas d'effet significatif de la présence d'une assistance haptique \myvari{1} sur le score \myacro{acr-RMSD} minimum atteint \myvard{1} (\myanova{exp4-rmsd-haptic-anova.tex}).
					...
					La \myref{fig-exp4-TempsPourAtteindreLeScoreRMSDMinimumAvecEtSansHaptique} présente le temps du score \myglos{acr-RMSD} minimum atteint \myvard{2} avec et sans haptique \myvari{1}.
					L'analyse montre qu'il n'y a pas d'effet significatif de la présence d'une assistance haptique \myvari{1} sur le temps du score \myacro{acr-RMSD} minimum atteint \myvard{2} (\myanova{exp4-rmsd-time-haptic-anova.tex}).
					...
					La \myref{fig-exp4-TempsPourAtteindreLeScoreRMSDMinimumAvecEtSansHaptiquePourChaqueScenario} présente le temps du score \myglos{acr-RMSD} minimum atteint \myvard{2} avec et sans haptique \myvari{1} pour chaque scénario.
					L'analyse montre qu'il n'y a pas d'effet significatif de la présence d'une assistance haptique \myvari{1} sur le temps du score \myacro{acr-RMSD} minimum atteint \myvard{2} pour la molécule \myUbiquitin (\myanova{exp4-rmsd-time-molecule-haptic-ubiquitin-anova.tex}).
					L'analyse montre un effet significatif de la présence d'une assistance haptique \myvari{1} sur le temps du score \myacro{acr-RMSD} minimum atteint \myvard{2} pour le complexe de molécules \myNusENusG (\myanova{exp4-rmsd-time-molecule-haptic-nuse-anova.tex}) inférieur de \myratio{exp4-rmsd-time-molecule-haptic-nuse-ratio.tex}.
					...
					La \myref{fig-exp4-NombreDeSelectionsParSecondeEffectueesParUnOperateurPourLaDeformationAvecEtSansHaptique} présente le nombre de sélections par seconde effectuées par les opérateurs pour une déformation \myvard{3} avec et sans haptique \myvari{1}.
					L'analyse montre un effet significatif de la présence d'une assistance haptique \myvari{1} sur le nombre de sélections \myvard{3} (\myanova{exp4-freq-button1-haptic-anova.tex}) inférieur de \myratio{exp4-freq-button1-haptic-ratio.tex}.
					...
					La \myref{fig-exp4-TempsMoyenDUneSelectionParSecondeEffectueeParUnOperateurPourLaDeformationAvecEtSansHaptique} présente le temps moyen d'une sélection effectuée par un opérateur pour une déformation avec et sans haptique \myvari{1}.
					L'analyse montre un effet significatif de la présence d'une assistance haptique \myvari{1} sur le temps moyen d'une sélection (\myanova{exp4-meantime-button1-haptic-anova.tex}) supérieur de \myratio{exp4-meantime-button1-haptic-ratio.tex}.
					...
					La \myref{fig-exp4-TempsMoyenPourAtteindreUneCibleDesigneeLorsDUneDesignationAvecEtSansHaptique} présente le temps moyen mis par un opérateur pour atteindre une cible acceptée lors du processus de désignation avec et sans haptique \myvari{1}.
					L'analyse montre un effet significatif de la présence d'une assistance haptique \myvari{1} sur ce temps moyen (\myanova{exp4-attract-time-haptic-anova.tex}) inférieur de \myratio{exp4-attract-time-haptic-ratio.tex}.
				\end{mysubsubsection}
				\begin{mysubsubsection}[sss-exp4-AmeliorationDesPerformances-AnalyseEtDiscussion]{Analyse et discussion}
					...
					D'ailleurs, la \myref{fig-exp4-TempsPourAtteindreLeScoreRMSDMinimumAvecEtSansHaptique} montre que le temps mis pour atteindre ce score ne s'est également pas amélioré avec les outils d'assistance haptique proposés.

					Cependant, une analyse tenant compte des spécificités de chaque scénario montre que l'utilisation des outils d'assistance haptique permet une amélioration sur le complexe de molécules \myNusENusG (scénario~\myscenario{2}) mais pas sur la molécule \myUbiquitin (scénario~\myscenario{1}).
					Le complexe de molécules \myNusENusG est la tâche la plus complexe à réaliser part le nombre de \myglos*{glo-Residu} à déformer et part la taille globale des deux molécules en présence.
					...
					La \myref{fig-exp4-TempsMoyenDUneSelectionParSecondeEffectueeParUnOperateurPourLaDeformationAvecEtSansHaptique}, qui montre que les outils d'assistance haptique allonge la durée des sélections, nous permet de déduire que les opérateurs produisent des déformations plus longues dans le temps.
			...
			\begin{mysubsection}[sse-exp4-AmeliorationDeLaCommunication]{Amélioration de la communication}
				\begin{mysubsubsection}[sss-exp4-AmeliorationDeLaCommunication-DonneesEtTestsStatistiques]{Données et tests statistiques}
					...
					La \myref{fig-exp4-TempsMoyenDAcceptationDUneDesignationAvecEtSansHaptique} présente le temps moyen mis par un opérateur pour accepter une désignation avec et sans haptique \myvari{1}.
					L'analyse montre un effet significatif de la présence d'une assistance haptique \myvari{1} sur ce temps moyen (\myanova{exp4-shake-time-haptic-anova.tex}) inférieur de \myratio{exp4-shake-time-haptic-ratio.tex}.
					...
					La \myref{fig-exp4-NombreDeDesignationsAccepteesAuCoursDuProcessusDeDesignationAvecEtSansHaptique} présente le nombre de désignations acceptées au cours du processus de désignation avec et sans haptique \myvari{1}.
					L'analyse montre un effet significatif de la présence d'une assistance haptique \myvari{1} sur ce nombre de sélections (\myanova{exp4-accept-haptic-anova.tex}) supérieur de \myratio{exp4-accept-haptic-ratio.tex}.
					...
					La \myref{fig-exp4-VitesseMoyenneDuCoordinateurAvecEtSansHaptique} présente la vitesse moyenne du coordinateur avec et sans haptique \myvari{1}.
					L'analyse montre un effet significatif de la présence d'une assistance haptique \myvari{1} sur ce nombre de sélections (\myanova{exp4-speed-haptic-anova.tex}) supérieur de \myratio{exp4-speed-haptic-ratio.tex}.
					...
					La \myref{fig-exp4-TempsDeParoleDesSujetsAvecEtSansHaptique} présente le temps de parole des sujets \myvard{4} avec et sans haptique \myvari{1}.
					L'analyse montre qu'il n'y a pas d'effet significatif de la présence d'une assistance haptique \myvari{1} sur le temps de parole \myvard{4} (\myanova{exp4-annotation-time-haptic-anova.tex}).
			...
			\begin{mysubsection}[sse-exp4-EvaluationQualitative]{Évaluation qualitative}
				\begin{mysubsubsection}[sss-exp4-QuestionnaireDUtilisabilite]{Questionnaire d'utilisabilité}
					Cette dernière expérimentation a été l'occasion d'évaluer l'utilisabilité de la plate-forme \myShaddock à l'aide du score \myacro{acr-SUS} décrit dans la \myref{sse-Questionnaires-QuatriemeExperimentation-LeQuestionnaireSUS}.
					L'évaluation par l'ensemble des utilisateurs fournit une note relativement peu élevée (\mysummary{exp4-evaluation-sus.tex}).
					Cependant, si on restreint ce score au utilisateurs biologistes, le score augmente (\mysummary{exp4-evaluation-sus-biologists.tex}).
					...
					En particulier, ils auraient souhaité pouvoir stabiliser un \myglos{glo-Residu} déjà placer ou encore pouvoir accéder à des structures moléculaires de taille plus importante que les \myglos*{glo-Residu} tels que les \myhelice* et les \myfeuillet*.
					Cependant, ce dernier point a été traité dans la \myref{sss-Shaddock-DeformationParGroupeDAtomes} où il est expliqué que cet outil n'a pas été développé principalement à cause de contraintes techniques.
				\end{mysubsubsection}
				\begin{mysubsubsection}[sss-exp4-QuestionnaireSurLaConsciencePeripherique]{Questionnaire sur la conscience périphérique}
					Les utilisateurs sont relativement conscients des actions de leurs partenaires puisqu'ils estiment comprendre les requêtes et consignes de leurs collègues rapidement (\mysummary{exp4-evaluation-resume-2.tex}) et qu'ils sont conscients à chaque instant de la position des autres collaborateurs (\mysummary{exp4-evaluation-resume-1.tex})\footnote{L'échelle de notation est comprise entre \mynum{1} et \mynum{5} mais les moyennes ont été normalisées entre \mynum{0} et \mynum{4}.}.
					D'ailleurs, ils considèrent que leurs partenaires font de même puisqu'ils n'éprouvent pas le besoin de se signaler (\mysummary{exp4-evaluation-resume-3.tex}).
					...
					Cependant, les utilisateurs éprouvent le besoin de se signaler dans un cas particulier : l'acceptation d'une requête ou d'une consigne (\mysummary{exp4-evaluation-resume-4.tex}).
					...
					Par exemple, le coordinateur peut indiquer oralement une cible qu'il a désigné mais qu'un opérateur n'aurait pas vu ou encore, un opérateur peut signaler un \myglos{glo-Residu}, ayant besoin d'être rapidement replacé, au coordinateur.
					...
					En effet, un test de \mycite[author]{Wilcoxon-1945} montre qu'il n'y a aucun effet significatif des retours haptiques sur :
					\begin{itemize}
					...
					Cependant, on note une évolution dans le rôle de l'assistance haptique lorsqu'on demande aux utilisateurs si les consignes peuvent être données sans utiliser de communication verbale (\mysummary{exp4-evaluation-resume-5.tex}).
					En effet, le test de \mycite[author]{Wilcoxon-1945} obtient un résultat assez proche du seuil de significativité (\myanova{exp4-evaluation-resume-5-anova.tex}).
		...
		\begin{mysection}[sec-exp4-Synthese]{Synthèse}
			\begin{mysubsection}[sse-exp4-ResumeDesResultats]{Résumé des résultats}
				...
				De plus, nous avons pu confronter la plate-forme \myShaddock à des bio-informaticiens, ayant une expertise dans le domaine du \myglos{glo-DockingMoleculaire}.
				...
				Ce dernier outil a été discuté dans la \myref{sss-Shaddock-DeformationParGroupeDAtomes} mais les contraintes de manipulation d'un tel ensemble d'atomes entraînerait des modifications importantes sur les métaphores de manipulation.
				En se basant sur les travaux de \mycite[author]{Bangor-2009}, on peut évaluer notre plate-forme comme \og \myemph{OK} \fg ce qui signifie qu'elle se trouve en-dessous de la moyenne globale d'utilisabilité du score d'évaluation \myacro{acr-SUS} mais qu'elle reste une application utilisable dans l'état.
