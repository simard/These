...
\logo{%
	\myimage[height=0.5cm]{logo-ups}%
	\hspace{1em}%
	\myimage[height=0.5cm]{logo-limsi}%
}
...
\begin{document}
	...
	\begin{frame}<1-9>[label=fra-synthesis]{Synthèse}
		\begin{myfigure}
	...
	\subsubsection{Objectifs}
	\begin{frame}{Objectifs}
		\begin{block}{Communication et coordination}
			\begin{itemize}
				\item Améliorer la coordination
				\item Réduire la communication de gestion de conflits
			\end{itemize}
		\end{block}
		\begin{block}{Stratégie de travail}
			\begin{itemize}
				\item Valider la configuration de travail
				\item Obtenir une répartition équitable du travail
			\end{itemize}
		\end{block}
		\begin{block}{Ergonomie}
			\begin{itemize}
				\item Valider les outils haptiques utilisés
				\item Évaluer l'outil auprès des utilisateurs
			\end{itemize}
		\end{block}
	\end{frame}
	\subsubsection{Synthèse}
