...
\begin{document}
	...
	\begin{myframe}{Définition du \mydocking moléculaire}
		\begin{myplusblock}{Définition}%
			ou \myemph{amarrage moléculaire}, consiste à trouver l'orientation et la conformation optimale permettant d'assembler deux molécules.%
		\end{myplusblock}%
		\vspace{-1ex}
		\begin{columns}[T]%
		...
		\end{columns}%
		\vspace{-1ex}
		\only<7>{\centering\alert{$\Rightarrow$ Résolution du \mydocking couteux en temps de calcul}\par}
	\end{myframe}
		...
		\begin{columns}
			...
			\begin{column}{0.475\textwidth}
				\begin{myblock}{Synthèse des approches existantes}
					...
					\end{itemize}
					\onslide<2>{\alert{$\Rightarrow$ \myDocking d'intérêt biologique ?}}
				\end{myblock}
			\end{column}
		\end{columns}
	\end{myframe}
	...
	\begin{myframe}{Organisation logicielle}
		\begin{myfigure}
			...
			\begin{myps}(-7.5,-7)(7.5,6)
				...
				\myumlinterface[angleA=0,outAngleA=0,angleB=180,outAngleB=180,offsetB=-8pt,ArrowInsidePos=0.4]{NAMD}{VMD}[nccurve]
				\myumlinterface[angleA=180,outAngleA=180,angleB=0,outAngleB=0,ncurvA=1.5,offsetA=12pt,ArrowInsidePos=0.6]{VRPN1}{VMD}[nccurve]
				\myumlinterface[angleA=180,outAngleA=180,angleB=0,outAngleB=0,ncurvA=0.67,ncurvB=0.33,offsetA=12pt,ArrowInsidePos=0.4]{VRPN2}{VMD}[nccurve]
			...
			\begin{column}{0.35\textwidth}
				...
				\begin{myplusblock}{Fonctionnalités}
					\begin{itemize}
						...
						\item<7-> Pointage visuel
						\item<8-> Différentes niveaux de sélection
					\end{itemize}
				\end{myplusblock}
			\end{column}
			\begin{column}{0.6\textwidth}
			...
			\begin{column}{0.5\textwidth}%
				\only<1-7>{%
					\begin{myblock}{Description}%
						...
						%						\begin{description}%
						%							\item<2->[Recherche] Identifier une cible (atome, résidu, \dots)%
				...
				\only<1-9>{%
					\begin{myfigure}%
						...
						\only<8>{%
							\mycaption[fig-EtapeDeRecherche]{Étape de recherche}%
						}%
						\only<9>{%
							\mycaption[fig-EtapeDeRecherche]{Étape de manipulation}%
						}%
					\end{myfigure}%
				}%
				\vspace{-5ex}%
				\only<8-9>{%
					\begin{myfigure}%
						...
						\end{myps}%
						\only<8>{%
							\mycaption[fig-EtapeDeSelection]{Étape de sélection}%
						}%
						\only<9>{%
							\mycaption[fig-EtapeDEvaluation]{Étape d'évaluation}%
						}%
					\end{myfigure}%
				}
			\end{column}%
	...
	\begin{myframe}{Amélioration des performances en collaboration}
		...
		\begin{myblock}{Synthèse}
			\begin{itemize}
				...
				\item<5-> Répartition différente du temps selon les tâches
			\end{itemize}
		\end{myblock}
	\end{myframe}
	\begin{myframe}{Stratégies de travail}
		...
		\begin{myblock}{Synthèse}
			\onslide<2->{
				Trois stratégies liées à l'affinité entre les collaborateurs
				\begin{description}
			...
			\begin{column}{0.65\textwidth}
				...
				\begin{myfigure}
					\begin{myps}(-1,0)(12,9)
						...
						\rput(7.7,7.4){\rnode{deformed-residue-label}{\textcolor{myblue}{résidu courant}}}
						\cnode(2.3,1.6){0.8}{ghost-residue}
						\rput(0.5,2.75){\rnode{ghost-residue-label}{\textcolor{myblue}{\mymultiline{résidu\\cible}}}}
	...
	\begin{myframe}{Influence de la complexité de la tâche}
		...
		\end{mytable}
		\vspace{-4ex}
		\begin{myblock}{Résultat important}
			Certaines manipulations nécessitent de la coordination
		\end{myblock}
		...
		\begin{columns}[T]
			\begin{column}{0.65\textwidth}
				\begin{myfigure}
					...
					\begin{myboxgraph}[llx=-3em,yAxisLabelPos={-3em,c}](2,0.8\textwidth)[0.5]{scénario}(1.75,1.9cm){vitesse~(mm/s)}
		...
		\begin{columns}[T]
			\begin{column}{0.65\textwidth}
				\begin{myfigure}
					...
					\end{myboxgraph}
					\mycaption[fig-exp4-FrequenceDeSelectionDUnOperateur]{Fréquence de sélections d'un opérateur}
				\end{myfigure}
			\end{column}
			\begin{column}{0.3\textwidth}
				\begin{myblock}{Synthèse}
